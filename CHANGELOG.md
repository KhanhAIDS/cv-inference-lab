...
- 2026-07-20 11:28:06 +07:00
  - Mục tiêu: hoàn tất phần còn lại Giai đoạn 2 (pilot resolution-confound, profiling Modal L4) + bắt đầu Giai đoạn 3 (cache FIgLib dev) + thêm gate an toàn Giai đoạn 5 sớm (code sẵn trước khi cần dùng).
  - Pilot resolution-confound RF-DETR/D-Fire: chạy `eval.py detector-cache --backend rfdetr --pilot` cho `rfdetr_large_dfire` ở cả `imgsz=800` và `imgsz=1280` trên 30 frame FIgLib dev pilot (diverse sequence/camera). Kết quả: 0 NaN, 0 zero-size box cả 2 resolution; 1-2 box lớn (~50-60% diện tích frame) khớp cùng 1 frame/vị trí giữa 2 resolution (verify: sequence `20160604_FIRE_smer-tcs3-mobo-c` offset `+2400s`, xyxy gần trùng) — là smoke plume thật lớn, không phải lỗi degenerate. **Pilot PASS** → giữ nguyên plan mặc định, không chuyển native-resolution.
  - Deploy + validate Modal L4 profiling (`profile_yolo_candidate`, `profile_rfdetr_candidate`): test nhỏ (warmup 3/measured 5) cả 2 backend PASS lần 2 (lần 1 rfdetr crash, xem bug dưới). GPU xác nhận `NVIDIA L4`, VRAM `23.66 GB`, framework/torch/cuda version đúng.
  - Bug phát hiện + fix: `eval.py` có `from ultralytics import YOLO` không điều kiện ở top-level module — khi chạy trên `rfdetr_image` (không cài `ultralytics`) toàn bộ script crash `ModuleNotFoundError` dù chỉ dùng backend rfdetr. Sửa thành lazy import trong `cmd_accuracy` và nhánh yolo của `load_backend`. Test lại `profile_rfdetr_candidate` PASS sau fix.
  - Upload lên Modal volume `smoke-fire-step13-volume` (3 checkpoint thiếu để chạy profiling): `runs/yolo26x_dfire/weights/best.pt`, `runs/rfdetr_large_dfire/checkpoint_best_total.pth`, `runs/rfdetr_large_pyro_sdis/checkpoint_best_total.pth`. Xóa lại 2 file test tạm trên volume sau khi verify (`profile_test.json`, `profile_test_rfdetr.json`).
  - Phát hiện (KHÔNG tự xử lý, ghi nhận để user quyết định): volume Modal còn nhiều run dir cũ chưa dọn theo đợt cleanup local 2026-07-20 (`yolo26x_pyro_sdis_budget9`, `yolo26x_pyro_sdis_smoke_localstage`, `_smoke_locked`, `_archive_benchmark`, `_warmstart`, `_resume_gate`, `resume_gate`) — ngoài scope plan hiện tại, không tự xóa.
  - Launch nền: `profile_yolo_candidate`/`profile_rfdetr_candidate` cho 6 candidate (resolution chính: `1280` cho 4 sạch+control, `1024` cho Pyronear; batch 1, 30 warmup + 300 measured x3) — output `artifacts/smoke_fire_detection/profile_<candidate_id>.json` trên volume.
  - Launch nền Giai đoạn 3: `eval.py detector-cache` cho 8 lượt cache FIgLib dev còn thiếu (GB10 local, `--resume`, batch 16): `rfdetr_large_pyro_sdis`, `yolo26x_dfire`, `rfdetr_large_dfire`, `yolo26n_dfire_control` (đều `imgsz=1280`), `pyronear_yolov8s_reference` (`imgsz=1024` main), cộng `yolo26x_dfire`/`rfdetr_large_dfire` phụ `imgsz=800`, `pyronear_yolov8s_reference` phụ `imgsz=1280`.
  - Thêm gate an toàn Giai đoạn 5 (được duyệt sẵn trong plan, xây trước khi cần — không phải scope creep): `eval.py detector-cache --split test` đọc `--winner-lock` (mặc định `figlib_dev_winner_lock.json`), reject nếu `--candidate-id` không phải `winner_candidate_id`/`allowlist_candidate_ids`. Không thêm manifest phụ, chỉ vài dòng check + đọc 1 JSON đã có kế hoạch tạo ở Giai đoạn 4.
