...
## 2026-07-21 09:03 GMT+7 — Giai đoạn 4 hoàn tất: winner-lock, resolution check, report tĩnh

- Bối cảnh: user quay lại server (không đi làm từ nhà được do không VPN/không SSH nội bộ — context server vẫn mới nhất). Kiểm tra: toàn bộ job nền (Giai đoạn 3: 8 cache dev; Giai đoạn 4: compare-matrix) đã chạy xong qua đêm không cần giám sát, đúng như thiết kế persistent (systemd+linger).
- Verify trạng thái trước khi làm tiếp: `wc -l` toàn bộ 8 cache dev = 28,360/28,360; `ls` xác nhận `figlib_dev_compare_matrix.json` đã tồn tại (job persistent tự hoàn tất, tạo lúc 2026-07-20 21:05 GMT+7); `git status`/`git log` xác nhận local khớp `origin/master`.
- Command đã chạy (đọc kết quả compare-matrix có sẵn, không chạy lại):
  - Đọc `figlib_dev_compare_matrix.json` bằng `python -m json.tool` → winner = `rfdetr_large_dfire` (AUROC 0.8317, thắng cả 3 rival đúng rule).
  - `.venv/bin/python -m tasks.smoke_fire_detection.temporal_eval temporal --cache artifacts/smoke_fire_detection/figlib_detector_cache_rfdetr_large_dfire_dev.jsonl --score smoke --out artifacts/smoke_fire_detection/figlib_dev_temporal_rfdetr_large_dfire.json` (chạy nền, ~9 phút) → chọn 2 operating point theo rule (lọc `false_alarm_rate_per_hour` ≤ budget quy đổi từ ngày/tuần, max `event_recall`, tie→min `ttd_median_seconds`, tie→min FA).
  - `.venv/bin/python -m tasks.smoke_fire_detection.temporal_eval g0 --cache artifacts/smoke_fire_detection/figlib_detector_cache_yolo26x_dfire_dev_res800.jsonl --out artifacts/smoke_fire_detection/gate_g0_auroc_yolo26x_dfire_res800.json` → AUROC@800 = 0.7308 (vs @1280 = 0.7605).
  - `.venv/bin/python -m tasks.smoke_fire_detection.temporal_eval g0 --cache artifacts/smoke_fire_detection/figlib_detector_cache_rfdetr_large_dfire_dev_res800.jsonl --out artifacts/smoke_fire_detection/gate_g0_auroc_rfdetr_large_dfire_res800.json` → AUROC@800 = 0.8256 (vs @1280 = 0.8317). **Kết luận câu hỏi bắt buộc: `@800` không vượt `@1280` cho cả 2 model — không có oversight, không đổi winner.**
  - `.venv/bin/python -m tasks.smoke_fire_detection.temporal_eval g0 --cache artifacts/smoke_fire_detection/figlib_detector_cache_yolo26n_dfire_control_dev.jsonl --out artifacts/smoke_fire_detection/gate_g0_auroc_yolo26n_dfire_control.json` → AUROC 0.7380 (control, không vượt winner).
  - `.venv/bin/python -m tasks.smoke_fire_detection.temporal_eval g0 --cache artifacts/smoke_fire_detection/figlib_detector_cache_pyronear_yolov8s_reference_dev.jsonl --out artifacts/smoke_fire_detection/gate_g0_auroc_pyronear_yolov8s_reference.json` → AUROC 0.8078 (leakage-caveat reference, không vượt winner).
  - `.venv/bin/python -m unittest tasks.smoke_fire_detection.test_eval tasks.smoke_fire_detection.test_temporal_eval -v` (31 test, pass) — chạy lại trước khi commit.
- Retention cleanup theo `post_train_benchmark_plan.md` ("Xóa ... secondary-resolution cache sau khi số đã chốt"): xóa `figlib_detector_cache_yolo26x_dfire_dev_res800.jsonl(.errors/.meta)`, `figlib_detector_cache_rfdetr_large_dfire_dev_res800.jsonl(.errors/.meta)`, `figlib_detector_cache_pyronear_yolov8s_reference_dev_res1280.jsonl(.errors/.meta)` — số đã chốt vào `gate_g0_auroc_*_res800.json` trước khi xóa raw. Xóa `tasks/smoke_fire_detection/__pycache__/` (tự sinh, gitignore).
- File trực tiếp tạo: `artifacts/smoke_fire_detection/figlib_dev_winner_lock.json`, `tasks/smoke_fire_detection/docs/model_benchmark_2x2.md` (report tĩnh đầy đủ: protocol, source-domain metrics, dev 2×2 matrix, pairwise/factorial/interaction, winner rule, resolution sensitivity, latency/VRAM, operating points, so sánh SOTA, caveat, việc chưa làm, bước tiếp theo).
- File trực tiếp sửa: `agent_context.md` (nén Giai đoạn 0-4 thành DONE summary, xóa checklist thực thi đã hoàn thành, thêm "Next — Giai đoạn 5"); `research_plan.md` (13d done, số chốt inline); `external_assets.md` (thêm 4 cache dev sạch + control/reference + artifact Giai đoạn 4 + report vào danh sách giữ); `CHANGELOG.md` (mục này).
- File trực tiếp xóa: 3 cache secondary-resolution + 6 sidecar (liệt kê trên), `tasks/smoke_fire_detection/__pycache__/`.
- File gián tiếp tạo (lệnh CLI tự ghi): `figlib_dev_temporal_rfdetr_large_dfire.json`, `gate_g0_auroc_yolo26x_dfire_res800.json`, `gate_g0_auroc_rfdetr_large_dfire_res800.json`, `gate_g0_auroc_yolo26n_dfire_control.json`, `gate_g0_auroc_pyronear_yolov8s_reference.json`.
- Việc CHỦ ĐỘNG CHƯA làm (ghi rõ trong report mục 11, không phải bỏ sót): human-visibility stratification, spatial-persistence overlay restore/hủy, lát cắt lỗi L0 trên winner, roadmap P1-P3, RQ5, SmokeyNet reference reproduction.
- **CHƯA chạy Giai đoạn 5** (`eval.py detector-cache --split test`, one-shot) — đang chờ user duyệt tường minh trước khi mở.
